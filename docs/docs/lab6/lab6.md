# Aufgabe 6 Automatisierte Governance mittels Fitness Functions
...
## Aufgabe 6.1 ArchUnit Clean Architecture Template

Binde folgenden Code, ohne in verstehen zu wollen, in dein Projekt ein:
...
Binde folgenden Code in dein Projekt ein. Versuche zu verstehen, was hier passiert und führe den Tests aus.
Zuvor musst du deinen Root Package Name angeben (anstelle von <YOUR ROOT PACKAGE>). Wenn noch nicht vorhandne, muss du ebenfalls
ArchUnit als Abhängigkeit in deinem Build Tool (Maven/Gradle) hinzufügen.
```xml
<dependency>
...
## Aufgabe 6.2 ArchUnit Boardmittel
